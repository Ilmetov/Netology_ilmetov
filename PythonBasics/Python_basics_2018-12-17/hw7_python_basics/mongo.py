...
agg_filename = 'C:\\Users\\doktor\\PycharmProjects\\Netology_ilmetov\\PythonBasics\\Python_basics_2018-12-17\\hw7_python_basics\\keywords.tsv'
tag_data = []
if db.tags.count() > 0:
    with open(agg_filename, 'r', encoding='utf-8') as f:
        for line in f.readlines():
            id, name = line.strip().split('/t')
            tags = eval(tags)
            for tag in tags:
                tag.update({'id': id})
                tag_data.append(
